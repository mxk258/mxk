    ...
    for (var m = 0; m < k.length; m++) {
        ...
        if (c == "left") {
            ...
            if (c == "center") {
                var p = g.x + g.w / 2;
                if (a.name != "linker") {
                    var r = {x: Math.round(p - a.props.w / 2)};
                    var l = Designer.op.changeShapeProps(a, r);
                    Utils.showAnchors(a);
                    Utils.mergeArray(b, l)
                } else {
                ...
                if (c == "right") {
                    ...
                    if (a.name != "linker") {
                        var q = Utils.getShapeBox(a);
                        var r = {x: n - a.props.w - (a.props.x - q.x)};
                        var l = Designer.op.changeShapeProps(a, r);
                        Utils.showAnchors(a);
                        Utils.mergeArray(b, l)
                    } else {
                    ...
                    if (c == "top") {
                        ...
                        if (c == "middle") {
                            var s = g.y + g.h / 2;
                            if (a.name != "linker") {
                                var r = {y: Math.round(s - a.props.h / 2)};
                                var l = Designer.op.changeShapeProps(a, r);
                                Utils.showAnchors(a);
                                Utils.mergeArray(b, l)
                            } else {
                            ...
                            if (c == "bottom") {
                                ...
                                if (a.name != "linker") {
                                    var q = Utils.getShapeBox(a);
                                    var r = {y: f - a.props.h - (a.props.y - q.y)};
                                    var l = Designer.op.changeShapeProps(a, r);
                                    Utils.showAnchors(a);
                                    Utils.mergeArray(b, l)
                                } else {
...
Designer.addFunction("setSchema", function (schemaCategories, callback) {
    ...
    Util.ajax({url: "/diagraming/schema", data: {categories: schemaCategories}, type: "get", success: function (data) {
        Schema.empty();
        eval(data);
        Schema.init(true);
        Designer.initialize.initShapes();
