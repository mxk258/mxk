...
$(function () {
    ...
    $("[title],[original-title]").live("mouseover", function () {
        ...
        if (k.attr("title_pos") == "right") {
            j.css({left: k.offset().left + k.outerWidth() + 7, top: k.offset().top + k.outerHeight() / 2 - j.outerHeight() / 2});
            $(".tip_arrow").attr("class", "tip_arrow tip_right").css("top", j.outerHeight() / 2 - 7)
        } else {
            if (k.attr("title_pos") == "top") {
                j.css({left: k.offset().left + k.outerWidth() / 2 - j.outerWidth() / 2, top: k.offset().top - j.outerHeight()});
                $(".tip_arrow").attr("class", "tip_arrow tip_top")
            } else {
                if (k.attr("title_pos") == "left") {
                    j.css({left: k.offset().left - j.outerWidth() - 7, top: k.offset().top + k.outerHeight() / 2 - j.outerHeight() / 2});
                    $(".tip_arrow").attr("class", "tip_arrow tip_left")
                } else {
                    j.css({left: k.offset().left + k.outerWidth() / 2 - j.outerWidth() / 2, top: k.offset().top + k.outerHeight()});
    ...
    $(".user_quickinfo").live("mouseenter", function () {
        ...
        i = setTimeout(function () {
            f = Util.ajax({url: "/u/quickinfo", data: {userId: j}, success: function (m) {
                ...
                $("#userinfo_follow_btn").unbind().bind("click", function () {
                    if ($(this).hasClass("green")) {
                        Util.ajax({url: "/start/unfollow", data: {followId: j}, success: function (n) {
                            $("#userinfo_follow_btn").text("关注").removeClass("green").addClass("default")
                        }})
                    } else {
                        Util.ajax({url: "/start/savefollow", data: {followIds: j}, success: function (n) {
    ...
    $(".tag_quickinfo").live("mouseenter", function () {
        ...
        b = setTimeout(function () {
            h = Util.ajax({url: "/tags/quickinfo", data: {tag: j}, success: function (m) {
                if (m.result == "not_exists") {
                    return
                }
                var l = e();
                ...
                $("#taginfo_follow_btn").unbind().bind("click", function () {
                    if ($(this).hasClass("green")) {
                        Util.ajax({url: "/tags/unfollow", data: {tagName: j}, success: function (n) {
                            $("#taginfo_follow_btn").text("关注").removeClass("green").addClass("default")
                        }})
                    } else {
                        Util.ajax({url: "/tags/follow", data: {tagName: j}, success: function (n) {
...
$.fn.submitFormAjax = function (a) {
    ...
    $.ajax({url: a.url ? a.url : $(this).attr("action"), type: "POST", data: $(this).serialize(), success: function (c) {
        if (c.error == "error") {
...
Util.ajax = function (a) {
    ...
    var b = {type: "POST"};
    a = $.extend(b, a);
    return $.ajax({url: a.url, type: a.type, traditional: true, data: a.data, success: function (c) {
        if (c.error == "error") {
            $.simpleAlert("暂时无法处理您的请求，请稍候重试。", "error", 3000);
            if (a.error) {
                a.error(c)
...
Util.load = function (d, a, c, e, b) {
    $.ajax({url: a, type: "POST", dataType: "html", data: c, success: function (f) {
        if (f.error == "error") {
            $.simpleAlert("暂时无法处理您的请求，请稍候重试。", "error", 3000)
        } else {
            if (f.error == "notlogin") {
...
Util.get = function (a, b, c) {
    $.ajax({url: a, type: "GET", data: b, success: function (d) {
        if (d.error == "error") {
            $.simpleAlert("暂时无法处理您的请求，请稍候重试。", "error", 3000)
        } else {
            if (d.error == "notlogin") {
...
$.fn.disable = function (c, b) {
    ...
    for (var a = 0; a < $(this).length; a++) {
        ...
        $(d).unbind("mouseover.disable").bind("mouseover.disable", function () {
            ...
            e.css({width: $(this).outerWidth() + c, height: $(this).outerHeight() + 4, top: $(this).offset().top, left: $(this).offset().left});
            if (b) {
                e.css("z-index", b)
            }
...
Util.checkPrivateFileCount = function (a) {
    Util.ajax({url: "/view/getprivatefilecount", success: function (b) {
        if (b.filecount >= b.totalcount) {
            $.confirm({content: "私有存储空间已经不足<br><br><span>▪ 您只能创建公开文件，点击确定继续。</span><br>▪ 您也可以去 <a target='_blank' href='/support/privatefile'>扩容</a>", onConfirm: function () {
                a()
            }})
        } else {
            a()
...
(function (a) {
    ...
    a.mask = function (d) {
        ...
        if (d == "open") {
            if (b == 0) {
                ...
                c.css({width: a(window).width() + "px", height: a(window).height() + "px", filter: "alpha(opacity=60)"}).show();
                a(window).bind("resize.mask", function () {
                    c.css({width: a(window).width() + "px", height: a(window).height() + "px"})
                })
            }
            b++
        } else {
    ...
    a.fn.popMenu = function (c) {
        ...
        var h = {position: "left", fixed: false, offsetX: 0, offsetY: 0, zindex: 2, autoClose: true, closeAfterClick: false, autoPosition: true};
        var d = a.extend(h, c);
        var g = a(d.target);
        ...
        if (d.autoPosition && e + d.offsetY + i.outerHeight() > a(window).height() + a(document).scrollTop()) {
            i.css({top: a(window).height() - i.outerHeight() + a(document).scrollTop(), left: f + d.offsetX})
        } else {
    ...
    a.fn.tooltip = function (d, c, f) {
        ...
        e = a("p#p_toolTip");
        a("p#p_toolTip #img_toolTip_Arrow").css({position: "absolute", top: "5px", left: "-13px"});
        if (!f) {
            e.show()
        } else {
            e.fadeIn("fast")
        }
        e.css({left: a(this).offset().left + a(this).width() + 18, top: a(this).offset().top - 16})
    };
    ...
    a.fn.suggest = function (d) {
        var h = a(this);
        var g = {valueField: "value", width: h.outerWidth(), format: function (j) {
            return j.text
        }};
        var e = a.extend(g, d);
        if (!h.data("suggest")) {
    ...
    a.fn.switchbutton = function (d) {
        var j = {val: true, width: 82, height: 30, innerHeight: 22, innerWidth: 22, innerBackground: "#fff", enable: true, title: ""};
        var e = a.extend(j, d);
        var i = a(this), c = i.find(".switch"), g = i.find(".switch-left"), f = i.find(".switch-right");
    ...
    a.fn.errorTip = function (e, d) {
        ...
        a("body").append(c);
        g = a(".signin-error");
        g.css({left: a(this).offset().left - g.width(), top: a(this).offset().top + a(this).height() / 2 - 7, opacity: "0", filter: "alpha(opacity=0)"});
        a(this).addClass(f);
        if (a(this).offset().left < 200) {
            g.animate({left: a(this).offset().left + g.width(), top: a(this).offset().top + a(this).height() / 2 - 7, opacity: "0.7", filter: "alpha(opacity=70)"}, 200)
        } else {
            g.animate({left: a(this).offset().left - g.width() - 14, top: a(this).offset().top + a(this).height() / 2 - 7, opacity: "0.7", filter: "alpha(opacity=70)"}, 200)
        }
    };
...
function globalNewDialog(c, a, b) {
    ...
    if ($("#dialog_new_diagram").length == 0) {
        Util.ajax({url: "/diagraming/new_dialog", data: {}, success: function (d) {
            $("body").append(d);
