...
var CLB = {socket: null, clientId: null, url: "", listenTime: "", unloaded: false, init: function () {
    var b = Math.random();
...
}, connection: null, listenFlag: true, listen: function (a) {
    if (this.listenFlag == false || CLB.connection != null) {
        return
    }
    CLB.connection = $.ajax({url: "/diagraming/listen", data: {clientId: CLB.clientId, userId: userId, name: userName, subject: chartId, listenTime: CLB.listenTime}, type: "get", success: function (b) {
        CLB.connection = null;
    ...
    CLB.connection = null
}, sending: false, onlineStatus: "on", messageQueue: [], send: function (c) {
    var b = {userId: userId, clientId: CLB.clientId, subject: chartId};
    var a = $.extend(b, c);
    this.messageQueue.push(a);
    if (this.sending == false) {
        this.doSend()
    }
}, sendErrorCount: 0, localSaveCount: 0, doSend: function (c) {
    ...
    $.ajax({url: "/diagraming/msg", data: {msgStr: b, ignore: "msgStr", chartId: chartId}, cache: false, type: "post", timeout: 15000, success: function (e) {
        CLB.sending = false;
    ...
    var d = {userId: userId, clientId: CLB.clientId, subject: chartId};
    var c = $.extend(d, b);
    var a = JSON.stringify(c);
    $.ajax({url: "/diagraming/msg_directly", data: {msgStr: a, ignore: "msgStr", chartId: chartId}, cache: false, type: "post", success: function (f) {
        if (e) {
            e(f)
        }
    }})
...
}, offlineRemind: true, offline: function () {
    this.onlineStatus = "off";
    this.sending = false;
    if (this.offlineRemind) {
        UI.showTip("与服务器连接不稳定，文件暂时保存到本地，连接正常时会自动同步到服务器。<a href='javascript:' style='margin-left: 10px;color:#0080FF' onclick='CLB.notRemindOffline()'>不再提醒</a>", "left")
    }
    CLB.doSend()
}, online: function () {
    UI.hideTip();
    ...
    this.sending = true;
    $.ajax({url: "/diagraming/savelocal", data: {def: a, title: b, chartId: chartId, ignore: "def"}, cache: false, type: "post", success: function (d) {
        if (c) {
            c(d)
        }
        CLB.sending = false;
    ...
    var a = {action: "chat", message: b, name: userName};
    CLB.sendDirectly(a);
    this.appendChatMsg(userName, b, false);
    $("#chatting_edit").val("").focus()
}, appendChatMsg: function (c, d, b) {
    d = d.replace(/\n/g, "<br/>");
    $("#chat_messages").append("<li><span>" + c + "</span>:&nbsp;" + Utils.filterXss(d) + "</li>");
    $("#chat_messages").scrollTop(9999999);
    if (b && !$("#open_chat_btn").button("isSelected")) {
        var a = parseInt($("#chat_prompt").text()) + 1;
        $("#chat_prompt").show().text(a).animate({top: "-15px"}, 200).animate({top: "-9px"}, 200)
    }
}, setConfig: function (b, a) {
    $.ajax({url: "/diagraming/config", data: {field: b, value: a}, cache: false, type: "post", success: function () {
    }})
}};