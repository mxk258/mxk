	...
	function createBox(target){
		...
		function bindEvents(target){
			...
			panel.unbind('.datebox').bind('click.datebox', function(e){
				if ($(e.target).hasClass('datebox-form-a')){
					var index = parseInt($(e.target).attr('datebox-form-index'));
		...
		function setButtons(target){
			var panel = $(target).combo('panel');
			if (panel.children('div.datebox-form').length){return}
			var button = $('<div class="datebox-form"><table cellspacing="0" cellpadding="0" style="width:100%"><tr></tr></table></div>').appendTo(panel);
			var tr = button.find('tr');
			for(var i=0; i<opts.buttons.length; i++){
				var td = $('<td></td>').appendTo(tr);
				var btn = opts.buttons[i];
				var t = $('<a class="datebox-form-a" href="javascript:void(0)"></a>').html($.isFunction(btn.text) ? btn.text(target) : btn.text).appendTo(td);
				t.attr('datebox-form-index', i);
			}
